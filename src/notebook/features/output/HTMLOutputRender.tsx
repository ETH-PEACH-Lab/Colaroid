...
export const HTMLOutputRender = (props: CellProps) => {
  ...
  const [recording, setRecording] = React.useState(
    props.content.recording
      ? JSON.parse(props.content.recording)
      : { events: [], startTime: -1 }
  );
  // config
  const SPEED = 1;
  ...
  const processDocument = () => {
    const htmlDocuments = props.content.result.filter(
      (e) => e.format === "html"
    );
    ...
    jsResults.forEach((line) => {
      ...
      const jsDocument = props.content.result.filter(
        (e) => e.title === fileName
      );
    ...
    cssResults.forEach((line) => {
      ...
      const fileName = line[1].replace(/"|\\/g, "");
      const cssDocument = props.content.result.filter(
        (e) => e.title === fileName
      );
  ...
  function removeListener(eventName, handler) {
    ...
    return iframeDocument.removeEventListener(eventName, handler, true);
  }
  ...
  const startPlay = () => {
    ...
    (function draw() {
      ...
      let totalTime =
        recording.events[recording.events.length - 1].time -
        recording.startTime;
  ...
  const reload = () => {
    ...
    iframeWrapperRef.current.innerHTML =
      '<iframe class="output-wrapper"> <iframe/>';
    let iframeEle = iframeWrapperRef.current.childNodes[0] as HTMLIFrameElement;
    iframeEle.addEventListener("load", onOutputLoad);
    let iframeDoc = iframeEle.contentDocument;
    const processedHTML = processDocument();
  ...
  return (
    <div>
      <div
        className="preview-cell-wrapper"
        id={`preview-cell-wrapper-${props.content.hash}`}
      >
        <div className="toolbar-container">
          <ul className="toolbar-wrapper">
            <li className="wrapper-button" onClick={reload} title="Refresh">
