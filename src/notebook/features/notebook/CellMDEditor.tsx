...
import { changeMarkdown, selectViewOption } from "./notebookSlice";
...
export function CellMDEditor(props: CellProps) {
  ...
  const keydownHandler = (e) => {
    ...
    if (e.key === "Enter" && keyPressed["Shift"]) {
      switchMode();
      dispatch(
        changeMarkdown({ hash: props.content.hash, message: markdownValue })
      );
  ...
  return (
    <div>
      <div
        className="md-cell-wrapper"
        id={`md-cell-wrapper-${props.content.hash}`}
      >
        {mode ? (
          <div
            ref={markdownEditorRef}
            className="md-cell-editor"
            id={`md-cell-editor-${props.content.hash}`}
            style={{ height: height }}
          ...
          <div
            ...
            className="md-cell-preview"
            id={`md-cell-preview-${props.content.hash}`}
            onDoubleClick={switchMode}
          >
            <MarkdownView
              markdown={props.content.message}
